...
namespace theta {
     ...
     *
     * @param nllilklihood is a function object which must implement double operator()(const double*) const which returns
     ...
     *   object.
     */
    template<class nlltype, class resulttype, class rndtype>
    void metropolisHastings(const nlltype & nllikelihood, resulttype &res, rndtype & rand,
    ...
    double factor = 2.38 / sqrt(npar_reduced);
        ...
        /*double * __restrict x_new = new double[npar];
        double * __restrict dx = new double[npar];*/
        //set the starting point:
        std::copy(startvalues.begin(), startvalues.end(), &x[0]);
        double nll = nllikelihood(x);
        ...
        for (size_t it = 1; it < iter; it++) {
            ...
            double nll_new = nllikelihood(x_new);
            if ((nll_new <= nll) || (rand.uniform() < exp(nll - nll_new))) {
                if(it > burn_in){
                    res.fill(x, nll, weight);
                    weight = 1;
                }
                std::swap(x, x_new);
                nll = nll_new;
