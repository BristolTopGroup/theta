...
/*
 * This header file defines the interface for the Histogram Plugin to be implemented
 * by files compiled to .so which can then be used by the theta framework.
 */
namespace theta{
    namespace plugin{
...
public:
    ...
    /// The setting to be used for constructing the object:
...
public:
    ConfigurationContext(boost::shared_ptr<VarIdManager> & vm_, const libconfig::Setting & rootsetting_, const libconfig::Setting & setting_,
    ...
    */
    ConfigurationContext(const ConfigurationContext & cfg, const libconfig::Setting & setting_): vm(cfg.vm),
 ...
 *    of the new class from a <tt>ConfigurationContext</tt>. DistributionPlugin::getTypeName must return a
...
public:
    ...
    virtual std::auto_ptr<product> build(ConfigurationContext & ctx) const = 0;
