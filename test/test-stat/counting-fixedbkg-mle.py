...
for mu in (2000.0, 3.0):
    for Theta in (5.0, 10000.0):
        ...
        query = "select count(*) from products where (writer__n_events_o > %(mu)g and abs(%(mu)g + mle__Theta - writer__n_events_o) > %(tol)g) or "\
                   "(writer__n_events_o <= %(mu)g and abs(mle__Theta) > %(tol)g)" % {'mu': mu, 'tol': 1e-3 * stat_error} 
        ...
        if Theta > 5000:
            rows = sql("counting-fixedbkg-mle.db", "select count(*) from products where abs(mle__Theta_error * mle__Theta_error - mle__Theta - %(mu)g) > %(tol)g" % {'mu': mu, 'tol': 1e-3 * stat_error})
            num_fail = rows[0][0]
